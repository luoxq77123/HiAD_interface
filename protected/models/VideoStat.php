 ...
 *
 */
class VideoStat extends CActiveRecord {
    ...
    public function __construct($date = '') {
        CActiveRecord::$db = Yii::app()->db_stat_video;
        if ($date != null) {
            $this->_date = $date;
            parent::__construct();
        }
    ...
    public static function model($date = '') {
        if (isset(self::$_models[$date])) {
            ...
            $model = self::$_models[$date] = new VideoStat(null);
            $model->_date = $date;
    ...
    private function getTableName($date) {
        $table_name = "video_$date";
        $table_name = in_array($table_name, $this->getTables()) ? $table_name : "video";
        return "{{".$table_name."}}";
    }
    
    private function getTables(){
        $cache_name = md5('model_VideoStat_getTables');
