...
class AppPositionTable extends CWidget {
    ...
    public function run() {
        ...
        if (isset($_GET['appGroupId']) && $_GET['appGroupId'] != '') {
            $sites = App::model()->findAllByAttributes(array('app_group_id' => $_GET['appGroupId']), 'status = 1');
            $site_ids = $sites ? CHtml::listData($sites, 'id', 'id') : array();
            $criteria->addInCondition('app_id', $site_ids);
        ...
        $this->render('appPositionTable', $set);
    }
