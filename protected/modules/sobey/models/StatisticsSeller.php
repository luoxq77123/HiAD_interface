<?php
class StatisticsSeller extends CActiveRecord {
    public $id;
    public $name;
    public $ctr;
    ...
    public function getAll($arrId, $adType=1, $startDate, $endDate) {
        ...
        $criteria->select = 'seller_id as id,seller_id,show_num,click_num,unique_users,dedicgotd_ip,cpm_cost,cpc_cost,cost,dates';
        ...
        if (!empty($arrId)) {
            $criteria->addInCondition('seller_id', $arrId);
        }
    ...
    public function getPageList($arrId, $adType=1, $startDate, $endDate, $route=null) {
        $user = Yii::app()->session['user'];
        $criteria = new CDbCriteria();
        $criteria->select = 'seller_id as id,seller_id,sum(show_num) as show_num,sum(click_num) as click_num,sum(unique_users) as unique_users,sum(dedicgotd_ip) as dedicgotd_ip,sum(cpd_cost) as cpd_cost,sum(cpm_cost) as cpm_cost,sum(cpc_cost) as cpc_cost,sum(cost) as cost,dates';
