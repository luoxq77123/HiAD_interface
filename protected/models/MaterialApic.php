...
class MaterialApic extends CActiveRecord {
    ...
    public function getContentByMaterialIds($arrMaterialId) {
        ...
        if (!empty($data)) {
            foreach($data as $val) {
                ...
                $return[$val->material_id]['description'] = $val->description;
                $return[$val->material_id]['pic_x'] = $val->pic_x;
                $return[$val->material_id]['pic_y'] = $val->pic_y;
            }
        }
        return $return;
    }
