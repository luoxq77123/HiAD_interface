...
class MaterialText extends CActiveRecord {
    ...
    public function getByMaterialText($materialTest_id) {
        $text_id = explode(",",$materialTest_id);
        $criteria = new CDbCriteria();
        //$criteria->select = 'id,ad_id,cushion,com_id,uid,width,height,pos_x,pos_y,material';
        $criteria->addInCondition('material_id', $text_id);
        $materialText = $this->findAll($criteria);
        $return = array();
        if (!empty($materialText)) {
            foreach($materialText as $val) {
                $return[$val->material_id]['materialSrc'] = $val['text'];
            }
        }
        return $return;
    }
