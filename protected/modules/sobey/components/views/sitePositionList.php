...
<div class="san_nav">
    <ul class="fl san_list" id="san_list">
        ...
        <li><a href="<?php echo Yii::app()->createUrl('site/list');?>" class="load_frame">站点</a></li>
        <li><a href="<?php echo Yii::app()->createUrl('sitePosition/getCode'); ?>" class="load_frame">获取代码</a></li>
    </ul>
    <a href="<?php echo Yii::app()->createUrl('sitePosition/index', array('a' => 'table'));?>" style="line-height: 30px;" class="fr mr_40 load_frame">查看广告位投放视图&gt;&gt;</a>
</div>
<!--end 导航-->
...
<!--生成代码-->
<div class="bgline add"><div style="float:left; margin-top:10px;">广告位：</div><a href="<?php echo Yii::app()->createUrl('sitePosition/add');?>" id="add_position" title="新建广告位" class="iscbut cbut_jia"><span>新建广告位</span></a></div>
...
<div class="tpboder adbox">
    <table border="0" cellspacing="0" cellpadding="0" class="list_table" id="list_table">
        ...
        <th scope="col" width="15%" class="tpboder">站点</th>
        ...
        <td>
            <a href="<?php echo Yii::app()->createUrl('sitePosition/edit', array('id' => $one->Position->id));?>" class="edit_position" title="修改广告位 <?php echo $one->Position->name;?>">修改</a> |
            <a href="<?php echo Yii::app()->createUrl('ad/list', array('positionid' => $one->Position->id, 'position_name' => urlencode($one->Position->name)));?>" title="查看广告" class="load_frame">广告</a>  |
            <a href="<?php echo Yii::app()->createUrl('statistics/site', array('type' => 'position', 'ad_id' => $one->Position->id, 'ad_name' => urlencode($one->Position->name)));?>" class="load_frame">统计</a>
        </td>
        </tr>
        <?php endforeach;?>
          <?php else:?>
...
<script type="text/javascript">
    ...
    function user_search(){
        ...
        var url = '<?php echo Yii::app()->createUrl('sitePosition/list')?>?siteGroupId=<?php echo @$_GET["siteGroupId"];?>&siteId=<?php echo @$_GET["siteId"];?>&status='+search_status+'&type='+search_type+'&size='+search_size+'&name='+encodeURIComponent(search_name);
	...
	function position_status(status, uid){
            ...
            $.post(
                '<?php echo Yii::app()->createUrl('sitePosition/status');?>', 
                ...
                function(data){
                    if(data.code < 0){
                        ...
                        setTimeout('ajax_load("ggw_box", "<?php echo Yii::app()->getController()->getAction()->id == 'index' ? Yii::app()->createUrl('sitePosition/list') : Yii::app()->request->getUrl();?>", true);', 1000);
                    }
                },
                'json'
            );
