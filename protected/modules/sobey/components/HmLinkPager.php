...
class HmLinkPager extends CBasePager {
	...
	protected function createPageButton($label,$page,$class,$hidden,$selected)
	{
        ...
        $link = $selected ? $page+1 : CHtml::link($label,$this->createPageUrl($page));
    ...
    public function ajaxBindScript(){
        $script = '
            <script type="text/javascript">
                $("#'.$this->htmlOptions['id'].'").find("a").click(function(){
                    '.($this->refreshArea != '' ? 'ajax_load("'.$this->refreshArea.'", $(this).attr("href"));' :  'frame_load($(this).attr("href"));').'
                    return false;
                });
            </script>
        ';
