...
class Schedule extends CActiveRecord {
    ...
    public function getAll($com_id) {
        $cache_name = md5('model_schedule_getAll_'.$com_id);
        $list = Yii::app()->memcache->get($cache_name);
        if (!$list) {
            $data = $this->findAll(array(
                    'select'=>'id,name,client_company_id,position_id,multi_time,status,createtime,salesman_id',
            ...
            $list = array();
            foreach ($data as $one) {
                $list[$one->id]['id']=$one->id;
                $list[$one->id]['name'] = $one->name;
                $list[$one->id]['position_id'] = $one->position_id;
                $list[$one->id]['client_company_id'] = $one->client_company_id;
                $list[$one->id]['multi_time'] = $one->multi_time;
                $list[$one->id]['createtime'] = $one->createtime;
                $list[$one->id]['status'] = $one->status;
                $list[$one->id]['salesman_id']=$one->salesman_id;
            }
