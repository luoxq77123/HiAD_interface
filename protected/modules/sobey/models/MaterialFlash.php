...
class MaterialFlash extends CActiveRecord {
    ...
    public function getByMaterialFlash($materialFlash_id) {
        $flash_id = explode(",",$materialFlash_id);
        $criteria = new CDbCriteria();
        //$criteria->select = 'id,ad_id,cushion,com_id,uid,width,height,pos_x,pos_y,material';
        $criteria->addInCondition('material_id', $flash_id);
        $materialFlash = $this->findAll($criteria);
        $return = array();
